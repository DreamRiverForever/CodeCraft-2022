...
public class Base3 {
    ...
    int baseCost;
    Set<Integer> nightNode = new HashSet<>();
    ArrayList<TimeBean> tempDemandList;


    public Base3(Resources res) {
    ...
    public Set<Integer> getNightNode() {
        return nightNode;
    }
    ...
    public int[][][] globalHandle() throws CloneNotSupportedException {
        ...
        distributionAvg5(days, D_sumFlow, solution);
        ...
        if (safeIndex > 1) {
            ...
            transferFlowTypeTwo(days, D_sumFlow, solution, safeIndex, sumFlow, 1);
        }
    ...
    public void distributionAvg5(int days, int[][] D_sumFlow, int[][][] solution) {
        ...
        for (int idx = 0; idx < days; idx++) {
            ...
            for (Integer col : clients) {
                ...
                while (!queue.isEmpty()) {
                    ...
                    for (Integer node_id : node_Id) {
                        if (temp[2] != 0 && D_sumFlow[node_id][idx] >= temp[2]) {
                            if (bandWidthMap.get(id2Site.get(node_id)) - D_sumFlow[node_id][idx] + surplus_flow[temp[1]][temp[0]] > baseCost + bandWidthMap.get(id2Site.get(node_id)) / 10)
                                continue;
                            // 当前时刻当前节点剩余流量要减少
                            D_sumFlow[node_id][idx] -= temp[2];
                            // 流量分配记录更新
    ...
    public int getSafeIndex90(int days) {
        // 用于记录安全区下标，就是安全区后面的流量不要钱
        double index_1 = days * 0.9;
        // 向上取整
        int safeIndex = (int) Math.ceil(index_1);
        return safeIndex;
    }

    public void getMaxTenNode(int days, int[][] sumFlow){

        ArrayList<Integer> nodeID = new ArrayList<>();
        for (int i = 0 ; i < qosMap.length ; i++){
            if (sites2client.get(i).size() != 0) nodeID.add(i);
        }
        ...
        for (int id : nodeID){
            tempList.sort(new Comparator<Integer>() {
                @Override
                public int compare(Integer o1, Integer o2) {
                    return sumFlow[id][o1] - sumFlow[id][o2];
                }
            });
            int index = getSafeIndex90(days);
            int index2 = getSafeIndex(days);
            index = tempList.get(index);

            int c = sumFlow[id][index - 1];
            int c_2 = sumFlow[id][index2 - 1];

            c = c <= baseCost ? baseCost : c +  (c - baseCost) * (c - baseCost) / bandWidthMap.get(id2Site.get(id));
            c_2 = c_2 <= baseCost ? baseCost : c_2 + (c_2 - baseCost) * (c_2 - baseCost) / bandWidthMap.get(id2Site.get(id));

            res.add(new int[]{c_2 - c, id});
        }
    ...
    public void distributionMaxDays2(int days, int[][] D_sumFlow, int[][][] solution, int safeIndex) {
        ...
        for (int idx = 0; idx < maxLen.size(); idx++) {
            ...
            if (sites2client.get(i).size() != 0 && nightNode.size() < 10) nightNode.add(i);
            ...
            if (nightNode.contains(i)) safeIndex = getSafeIndex90(days);
            else safeIndex = getSafeIndex(days);
...
//                        if (idx == raw) continue;
                ...
                if (nightNode.contains(idx)) safeIndex = getSafeIndex90(days);
                else safeIndex = getSafeIndex(days);
                for (int k = safeIndex - 1; k >= 0 ; k--) {
                    ...
                    while (!queue.isEmpty() && d_flow > 0) {
                        ...
                        ArrayList<Integer> node_Ids = (ArrayList<Integer>) lists[arr[0]];

                        // 遍历这个流量对应的客户端所有的边缘节点
                        for (int node_Id : node_Ids) {
                            int safeIndex_2 = nightNode.contains(node_Id) ? getSafeIndex90(days) : getSafeIndex(days);
                            if (idx != node_Id && node_days[node_Id].get(safeIndex_2 - 1) != safeDaysId) {
                                // 计算当前节点可以迁移多少流量，就是95%流量减去当前流量
                                int temp_safeDaysId = node_days[node_Id].get(safeIndex_2 - 1);
    ...
    public void transferFlowType2(int days, int[][] D_sumFlow, int[][][] solution, int safeIndex, int[][] sumFlow, int c) {
        // 开始流量迁移
